...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext.prototype.login = function () {
        ...
        if (this.config.extraQueryParameter && this.config.extraQueryParameter.indexOf('login_hint') !== -1) {
            var self = this;
            this._renewIdToken(function (errorDesc, token, error) {
                if (!token) {
                    self._loginInteractive();
                }
            });
        }
        else {
            this._loginInteractive();
        }
    };
    ...
    * Initiates the login process by redirecting the user to Azure AD authorization endpoint.
    */
    AuthenticationContext.prototype._loginInteractive = function (loginStartPage) {
    ...
    */
    ...
    AuthenticationContext.prototype._renewIdToken = function (callback) {
        ...
        // remove the existing prompt=... query parameter and add prompt=none
        var urlNavigate = this._urlRemoveQueryStringParameter(this._getNavigateUrl('id_token', null), 'prompt');
        urlNavigate = urlNavigate + '&prompt=none';
        urlNavigate = this._addHintParameters(urlNavigate);
