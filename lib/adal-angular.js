    ...
    if (angular) {
        ...
        AdalModule.provider('adalAuthenticationService', function () {
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                        ...
                        if (requestInfo.stateMatch) {
                            if (requestInfo.requestType === _adal.REQUEST_TYPE.RENEW_TOKEN) {
                                ...
                                if (window.parent !== window) { // 
                                    _adal.verbose("Login response received in iframe");
                                    //get parent scope from iframe
                                    var parentScope = $window.parent.angular.element($window.frameElement).scope();

                                    if (token) {
                                        parentScope.$emit('adal:silentLoginSuccess', token);
                                        if (_adal.callback && typeof _adal.callback === 'function') {
                                            _adal.callback(errorDescription, token, error, tokenType);
                                        }
                                    }
                                    else if (error && errorDescription) {
                                        parentScope.$emit('adal:silentLoginFailure', errorDescription, error);
                                    }
                                }
                                else {
                                    updateDataFromCache(_adal.config.loginResource);
                ...
                $rootScope.$on('adal:silentLoginSuccess', function (event, token) {
                    updateDataFromCache(_adal.config.loginResource);
                    $rootScope.$apply();
                    $rootScope.$broadcast('adal:loginSuccess', token);
                });

                $rootScope.$on('adal:silentLoginFailure', function (event, error, errorDescription) {
                    updateDataFromCache(_adal.config.loginResource);
                    $rootScope.$apply();
                    $rootScope.$broadcast('adal:loginFailure', error, errorDescription);
                });
